...
shinyServer(function(input, output, session){
  ...
  tempds<-read.csv("../.env",header=FALSE,sep="=",stringsAsFactors = FALSE)
  ...
  observeEvent(input$Add_row_head, {
    ...
    showModal(modalDialog(title = "Add a new row",
                          textInput(paste0("Parameter", input$Add_row_head), "Parameter"),
                          textInput(paste0("Value", input$Add_row_head), "Value"), 
  ...
  observeEvent(input$go, {
    new_row=data.frame(
      Parameter=input[[paste0("Parameter", input$Add_row_head)]],
      Value=input[[paste0("Value", input$Add_row_head)]]
    )
  ...
  observeEvent(input$newValue,
               {
                 ...
                 DF=data.frame(lapply(newValue, function(x) t(data.frame(x))),stringsAsFactors = F)
